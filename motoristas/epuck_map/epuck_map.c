...
Robot_concepts robot_init() {
  ...
  Robot_concepts concepts;
  wb_robot_init();
  concepts.left_motor = wb_robot_get_device("left wheel motor");
  concepts.right_motor = wb_robot_get_device("right wheel motor");
  wb_motor_set_position(concepts.left_motor, INFINITY);
  wb_motor_set_position(concepts.right_motor, INFINITY);
  wb_motor_set_velocity(concepts.left_motor, 0.1 * MAX_SPEED);
  wb_motor_set_velocity(concepts.right_motor, 0.1 * MAX_SPEED);
  ...
  wb_position_sensor_enable(right_encoder, TIME_STEP);
    ...
    for(int i = 0; i < 8; i++){
      ...
      concepts.ps[i] = wb_robot_get_device(ps_id);
      wb_distance_sensor_enable(concepts.ps[i], TIME_STEP);
    }
  ...
  concepts.robot_position = wb_supervisor_node_get_field(robot_node, "translation");
  concepts.robot_rotation = wb_supervisor_node_get_field(robot_node, "rotation");
...
int main(int argc, char **argv) {
  ...
  while (wb_robot_step(TIME_STEP) != -1) {
    const double *position = wb_supervisor_field_get_sf_vec3f(settings.robot_position);
    const double *rotation = wb_supervisor_field_get_sf_rotation(settings.robot_rotation);

    for(int i = 0; i < 8; i++) {
      dist[i] = convert_intensity_to_meters(wb_distance_sensor_get_value(settings.ps[i]));
    }
    ...
    {
      wb_motor_set_velocity(settings.left_motor, 0.2 * MAX_SPEED);
      wb_motor_set_velocity(settings.right_motor, -0.2 * MAX_SPEED);
    }
    if ( !detect_obstacle_ahead(dist) )
    {
      wb_motor_set_velocity(settings.left_motor, 0.5 * MAX_SPEED);
      wb_motor_set_velocity(settings.right_motor, 0.5 * MAX_SPEED);
    }
  }
